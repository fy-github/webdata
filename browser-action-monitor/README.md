...
截图功能现在支持三档可选策略，位置在 `Browser Action Monitor 设置 -> 附件采集 -> 截图存储档位`：

- `轻量`：`64 MB`，JPEG 质量 `0.55`，最大宽度 `1280`
- `均衡（推荐）`：`256 MB`，JPEG 质量 `0.72`，最大宽度 `1600`
- `取证`：`1024 MB`，JPEG 质量 `0.86`，最大宽度 `1920`

### 行为说明

- 截图字节优先保存在扩展本地的 `IndexedDB` 中，动作元数据仍保存在扩展状态里。
- 已经同步到服务端的截图会继续保留在服务端，本地与服务端默认是双份保留。
- 当本地截图占用达到当前档位上限时，会自动关闭截图功能，避免继续挤占浏览器存储。
- 如果截图是因为 `storage-pressure` 被自动关闭，用户把档位切换到更高等级并保存后，系统会自动恢复截图采集。
- 如果截图是用户手动关闭，或因为连续失败被关闭，不会因为切换档位自动恢复。

### 清空数据范围

- Popup 中的“清空数据”会清空本地记录数据和本地截图附件。
- “清空数据”不会删除服务端已经保存的数据，也不会主动回收服务端附件文件。
